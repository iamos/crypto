# #define
ENC = 1
DEC = 0
KEY_ENC = 2
KEY_DEC = 3
sbox = [
		
	]
isbox = [
		
	]

rcon = [
	
]

def makeblock(input_string):
	buff = [0] * 16;
	for x in range(16):
		buff[x] = ord(input_string[x]);
	return buff;

def bprint(input_block):
	for x in range(len(input_block)//4):
		print(input_block[(4*x):(4*x)+4]);

def sbprint(input_block):
	for x in range(len(input_block)//4):
		print(chr(input_block[(4*x)]),chr(input_block[(4*x)+1]), chr(input_block[(4*x)+2]), chr(input_block[(4*x)+3]));

def hexbprint(input_block):
	for x in range(len(input_block)//4):
		print(hex(input_block[(4*x)]),hex(input_block[(4*x)+1]), hex(input_block[(4*x)+2]), hex(input_block[(4*x)+3]));

def subbytes(input_block,flag):
	buff = [0] * 16;

	if(flag == ENC):
		for x in range(16):
			buff[x] = sbox[input_block[x]];
	elif(flag == DEC):
		for x in range(16):
			buff[x] = isbox[input_block[x]];
	elif(flag == KEY_ENC):
		word = [0]*4
		for x in range(4):
			word[x] = sbox[input_block[x]];
		return word
	elif(flag == KEY_DEC):
		word = [0]*4
		for x in range(4):
			word[x] = isbox[input_block[x]];
		return word
	return buff;

def mixcolum(input_block,flag):
	matrix = [2,3,1,1,1,2,3,1,1,1,2,3,3,1,1,2];
	imatrix = [14,11,13,9,9,14,11,13,13,9,14,11,11,13,9,14];
	buff = [0] * 16;
	if (flag == ENC):
		buff = matrixmult(matrix, input_block)
	elif(flag == DEC):
		buff = matrixmult(imatrix,input_block)
	return buff

def matrixmult(A, B):
	buff = [0] * 16;
	for index in range(16):
		for offset in range(4):
			buff[index] ^= gf_mult(A[((4*(index//4))+offset)], B[((4*offset)+(index%4))]);
	return buff

def shiftrow(input_block, flag):
	buff = [0] * 16;
	if(flag == ENC):
		for x in range(16):
			buff[x] = input_block[(x//4)*4+((x+(x//4))%4)];
	elif(flag == DEC):
		for x in range(4):
			buff[x] = input_block[x]
		buff[4] = input_block[7]
		buff[5] = input_block[4]
		buff[6] = input_block[5]
		buff[7] = input_block[6]
		buff[8] = input_block[10]
		buff[9] = input_block[11]
		buff[10] = input_block[8]
		buff[11] = input_block[9]
		buff[12] = input_block[13]
		buff[13] = input_block[14]
		buff[14] = input_block[15]
		buff[15] = input_block[12]
	return buff;

def gf_mult(a,b):
	p = 0x00;
	for counter in range(8):
		if b & 1 :
			p^=a;
		hi_bit = a & 0x80
		a <<= 1;
		a &= 0xFF;
		if hi_bit == 0x80:
			a ^=0x1b;
		b >>=1;
	return p;

def addroundkey(input_block, key_block):
	buff = [0]*16;
	#key_block = makeblock(key);
	for index in range(16):
		buff[index] = input_block[index]^key_block[index]
	return buff

def key_xor(a,b):
	buffer = [0]*4;
	for i in range(4):
		buffer[i] = a[i] ^ b
	return buffer

def key_rotate(word):
	buffer = [0]*4;
	buffer[0] = word[1]
	buffer[1] = word[2]
	buffer[2] = word[3]
	buffer[3] = word[0]
	return buffer

def keyexpansion(input_key):
	expansion_key = [0] * 176;
	for i in range(16):
		expansion_key[i] = ord(input_key[i]);

	for i in range(16,176):
		#IF 16's mutiple
		if(i%16 == 0):
			rnd = i//16
			shiftbuffer = [0]*4
			for offset in range(4):
				shiftbuffer[offset] = expansion_key[i-4+offset]
			shiftbuffer = key_rotate(shiftbuffer)
			shiftbuffer = subbytes(shiftbuffer,KEY_ENC)
			shiftbuffer = key_xor(shiftbuffer,rcon[rnd])
			for offset in range(4):
				expansion_key[i+offset] = expansion_key[i-16+offset] ^ shiftbuffer[offset]
			i = i+4
		else:
			expansion_key[i] = expansion_key[i-4]^expansion_key[i-16];
	return expansion_key

def aes_module(plaintext, input_key, flag):
	expansion_key = keyexpansion(input_key)
	block = makeblock(plaintext)
	if( flag == ENC):		
		block = addroundkey(block, expansion_key[0:16])
		for rnd in range(1,10):
			block = subbytes(block,ENC)
			block = shiftrow(block,ENC)
			block = mixcolum(block,ENC)
			block = addroundkey(block, expansion_key[(16*rnd):(16*rnd)+16])
			#print("ENC  start:",(16*rnd)//4,"end:",((16*rnd)+16)//4-1)
		block = subbytes(block,ENC)
		block = shiftrow(block,ENC)
		block = addroundkey(block,expansion_key[160:176])
		#print(">>ENCRYPT")
		#hexbprint(block)
	elif( flag == DEC):
		block = addroundkey(block, expansion_key[160:176])
		for rnd in range(1,10):
			block = shiftrow(block,DEC)
			block = subbytes(block,DEC)
			block = addroundkey(block, expansion_key[160-(16*rnd):160-(16*rnd)+16])
			block = mixcolum(block,DEC)
			#print("DEC  start:", (160-(16*rnd))//4,"end:", (160-(16*rnd)+16)//4)
		block = subbytes(block,DEC)
		block = shiftrow(block,DEC)
		block = addroundkey(block,expansion_key[0:16])
	hexbprint(block)
	p = [0]*16
	for index in range(16):
		p[index] = chr(block[index])
	return "".join(p)

#key1 = "Hanyang.University.ERICA .Dept.of.Computer.Science.&.Engineering";
#plaintext1 = "Informaion.Security.and.Privacy.lab";

k = "D3KJGF9FJSIEIFKGJFIBM39GHKF9GLF9FLDLFKF09HGJWMCBLD0234JDKDZQD39"
p = "GFKGI20SD294JD9FJDK209SWJKODFXCJNKLFDJIGJKDFIGJDJKFL2DL"
cipher = aes_module(p,k,ENC)
print(cipher)
plain = aes_module(cipher,k,DEC)
print(plain)

